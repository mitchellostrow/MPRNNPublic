...
class MPEnv(ngym.TrialEnv):
    ...
    def render(self, mode='human', close=False):
        ...
        if self.viewer is None:
                self.viewer = rendering.Viewer(400, 200)
                self.viewer.set_bounds(0, 400, 0, 200)
        fixate = [[190,90],[190,110],[210,110],[210,90]]
        s1 = [[90,90],[110,90],[110,110],[90,110]]
        s2 = [[290,90],[310,90],[310,110],[290,110]]
        if self.in_period('fixation'):
            self.viewer.draw_polygon(fixate,color = (0,0,255))
            text = "fixation"
        elif self.in_period('choice'):
            self.viewer.draw_polygon(fixate,color = (239,0,255))
            text = "choice"
        
        if not self.in_period('fixation'):
           
            self.viewer.draw_polygon(s1, color=(255,0,0))
            self.viewer.draw_polygon(s2, color=(0,255,0))

        outline1 = [[70,70],[130,70],[130,130],[70,130],[70,70]]
        outline2 = [[270,70],[330,70],[330,130],[270,130],[270,70]]
        if self.in_period('choice'):
            if self.model_hist[-1] == 1:
                self.viewer.draw_polyline(outline2, color = (0,0,0))
            elif self.model_hist[-1] == -1:
                self.viewer.draw_polyline(outline1,color = (0,0,0))
            text = "choice"

        if self.in_period('outcome'):
            if self.opponent_action == 1:
                self.viewer.draw_polyline(outline2, color = (119,0,255))
            else: 
                self.viewer.draw_polyline(outline1,color = (119,0,255))
            text = "outcome"


        label = Label(text, font_size=36,
			x=100, y=100, anchor_x='left', anchor_y='bottom',
			color=(0, 0, 0, 0))
